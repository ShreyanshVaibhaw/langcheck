...
| 08 | Done (code) | main | Config/persistence/startup/single-instance/CLI (08a, tested) + native tray icon & menu + `--background` (08b, FFI compiled). **UI polish:** custom embedded app icon (indigo check), balloon/toast notifications on toggles, clearer menu labels, and a **live TSF-adapter toggle** in the tray (kill switch from the UI, persisted). Rich multi-page settings dialog → file-based config.toml for MVP (deferred). Tray runtime + UI a11y verification pending (visuals confirmed on a real desktop). **Config behaviour completed:** the `[performance]` knobs `decision_deadline_ms`/`max_token_chars` are now read by the coordinator instead of being ignored (commit 27f3109), and **suggest-only mode** (`mode = "suggest"`) now detects-but-never-replaces on both the MVP and TSF paths rather than silently auto-correcting like conservative (commit ac13667; the candidate popup UI remains post-MVP). |
...
| 12 | Scaffolding | main | Per-user install.ps1/uninstall.ps1 (no admin, %LOCALAPPDATA%, reversible startup, clean uninstall) + package.ps1 (assembles the versioned release zip; verified locally) + packaging/README release process. **TSF adapter wired in:** install ships langcheck_tsf.dll + optional `-RegisterTsf` (machine-wide, UAC); uninstall detects an existing registration and unregisters it (elevated, before deleting the DLL) — verified locally (copy + clean uninstall; TSF paths guarded). **CI emits a CycloneDX SBOM artifact** (Syft). Manual gates (code-signing cert, clean-machine install testing) pending. |
| 13 | Done (code) | main | `langcheck-tsf` cdylib COM text service — **HOST-VERIFIED WORKING: with the LangCheck TIP active in WordPad, typing `wierd ` auto-corrects to `weird ` (and `problm `→`problem `) via a TSF edit session; broker served 20+ evaluate requests.** This is the rich-editor case SendInput couldn't do. Remaining: broader app-matrix host testing + polish. **Registration done + verified end-to-end on a real desktop** (elevated register/unregister round-trip writes then cleanly removes the HKLM CLSID + TSF keyboard-TIP profile, no crash): fail-open no-op ITfTextInputProcessor + class factory + DllGetClassObject/DllCanUnloadNow; machine-wide registration via ITfInputProcessorProfiles/ITfCategoryMgr exposed as DllRegisterServer/DllUnregisterServer; broker `--register-tsf`/`--unregister-tsf` that self-elevate via UAC. **Same-user broker IPC done + tested:** wire protocol in `langcheck-core::ipc` (Request/Response, defensive encode/decode, 7 unit tests) + a same-user (SID-scoped DACL), local-only (PIPE_REJECT_REMOTE_CLIENTS), message-mode named-pipe transport in the standalone `langcheck-ipc` crate (kept minimal so the in-process adapter need not pull in `langcheck-windows`) with a real-pipe round-trip test. **Cross-process IPC verified on a real desktop** via diagnostic modes `--broker-serve`/`--broker-eval` (wierd→replace weird, teh→the, the→leave, ping→pong) — the exact mechanism the adapter will use. **Broker server wired:** the pipe server runs on a detached thread in `--background`, answering Evaluate via a shared engine decision (`crate::engine::decide`, now used by BOTH the keystroke coordinator and the IPC handler so they cannot diverge) and honouring the kill switch; engine handler unit-tested against the real lexicon (wierd→weird, known→leave, blocked→leave, non-prose→leave). **Findings:** (1) TSF TIP registration is machine-wide (HKLM) and needs admin — not per-user, like every IME; (2) the category manager must be created *after* the profile registration or RegisterCategory access-violates (stale COM ref); (3) ConnectNamedPipe returns ERROR_PIPE_CONNECTED (success) when a client races ahead. **In-DLL IPC client wired + verified:** the adapter links `langcheck-ipc` and asks the broker via `ask_broker(token,boundary)` (fail-open); a `LangCheckIpcSelfTest` export driven by `langcheck --tsf-selftest` loads the DLL into a process and confirms it pings + evaluates wierd→weird over the pipe to a separate broker — PASSED on a real desktop. **Focus sink wired + COM-harness-verified:** activation now advises a thread-manager event sink (ITfThreadMgrEventSink); `langcheck --tsf-comtest` drives activate→advise→create-doc→focus→deactivate through a real TSF thread manager with no host app and confirms it runs **clean, no fault** (catches the RegisterCategory-style AV class). **Read-only word detection wired:** on focus the sink advises a per-context ITfTextEditSink; its OnEndEdit reads the text before the caret (GetSelection→ShiftStart→GetText, fail-open) and, when a word is completed by a boundary, calls `ask_broker` — but does **NOT modify text yet** (read-only, so a bug cannot corrupt the editor). The pure detection logic (`detect_token`) is unit-tested; the broker (`--broker-serve`) prints a privacy-safe request **count** (never the token) so detection is host-verifiable. **Write path implemented:** OnEndEdit → ask_broker → on a correction, an async (TF_ES_ASYNCDONTCARE) read-write ITfEditSession SetText-replaces the word's range (TF_ST_CORRECTION). Safety: the word range is re-derived and its text re-checked to equal the detected token before any edit, so it can never replace the wrong text; fail-open throughout. The adapter is now **feature-complete pending host-process testing** — the live correction inside a real editor is the one thing not verifiable off-host (compile + unit + COM-harness clean here). **Coordination fixed (commit 5b9224c):** under `--background` both paths are live, and the low-level keyboard hook used to beat the adapter on the first word and garble rich editors (host-reported in Win11 Notepad: `wierd`→`wwd`); the adapter now sends a `Request::Active` focus beacon on `OnSetFocus` so the MVP keystroke path defers *before* it can fire (focus-scoped, so non-TSF apps still correct normally). `--background` now matches `--broker-serve` cleanliness — pending host re-test in Notepad. |
| 14 | Decided: deferred | main | Research outcome recorded in ADR-0009: defer optional context/grammar — no model ships; `context_mode` stays an enforced-off invariant. Cost (privacy surface, precision risk, resource budget, opt-in UX) isn't justified without a measured quality gap + a privacy-UX design. Reversible; rejecting it changes nothing in the spelling architecture (as the brief permits). |
