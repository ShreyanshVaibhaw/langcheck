...
| 13 | In progress | main | `langcheck-tsf` cdylib COM text service. **Registration done + verified end-to-end on a real desktop** (elevated register/unregister round-trip writes then cleanly removes the HKLM CLSID + TSF keyboard-TIP profile, no crash): fail-open no-op ITfTextInputProcessor + class factory + DllGetClassObject/DllCanUnloadNow; machine-wide registration via ITfInputProcessorProfiles/ITfCategoryMgr exposed as DllRegisterServer/DllUnregisterServer; broker `--register-tsf`/`--unregister-tsf` that self-elevate via UAC. **Findings:** (1) TSF TIP registration is machine-wide (HKLM) and needs admin — not per-user, like every IME; (2) the category manager must be created *after* the profile registration or RegisterCategory access-violates (stale COM ref). Still a NO-OP (no edit logic) + not registered by default. To come: thread-mgr/key sinks, edit-session range replacement, same-user broker IPC, then host-process testing of the edit path. |
| 14 | Not started |  |  |
