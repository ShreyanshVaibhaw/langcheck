...
| 13 | In progress | main | Foundation: `langcheck-tsf` cdylib COM text service — fail-open no-op ITfTextInputProcessor + class factory + DllGetClassObject/DllCanUnloadNow, builds langcheck_tsf.dll. Registration, edit-session replacement, broker IPC, and host-process verification still to come (post-MVP; not registered by default). |
| 14 | Not started |  |  |
